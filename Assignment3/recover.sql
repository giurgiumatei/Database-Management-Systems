...
CREATE OR ALTER PROCEDURE usp_add_swords_recover @name VARCHAR(50), @brand_id INT, @price INT, @stars INT, @provider_name VARCHAR(50) AS
BEGIN
	BEGIN TRY
	---first insert
	BEGIN TRAN
	...
	SELECT 'The first transaction completed succesfully'
	COMMIT TRAN
	---second insert
	BEGIN TRAN
	...
	SELECT 'The second transaction completed succesfully'
	COMMIT TRAN
	---third insert
	BEGIN TRAN
	...
	COMMIT TRAN
	SELECT 'The third transaction completed succesfully'
	END TRY
...
---test case happy
SELECT * FROM Swords
EXEC usp_add_swords_recover 'test',1,50,4,'test'
SELECT * FROM Swords

---test case error
SELECT * FROM Swords
EXEC usp_add_swords_recover 'test2',1,50,4,'Boeing'
SELECT * FROM Swords
SELECT * FROM Swords_Providers
